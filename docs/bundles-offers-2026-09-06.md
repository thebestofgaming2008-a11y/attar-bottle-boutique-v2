...
### Mobile order summary follow-up

On phones/tablets below 1024px, checkout now has a fixed bottom **Your order + total** bar. Tapping opens an accessible slide-up dialog containing the same summary as desktop. Desktop retains its sticky side panel. The dialog has a close button, outside/Escape dismissal, focus return, background scroll lock, reduced-motion support and an independently scrolling body capped at 85dvh. Opening does not autofocus the coupon field or summon the keyboard. Coupon errors open the summary and remain indicated on the collapsed bar; payment processing closes/disables the sheet. Footer spacing accounts for the bottom bar and device safe area.

Verified at 390×844, a short 390×450 viewport and 1440×1000 desktop, including open/close, invalid code/removal, focus restoration, resize cleanup and payment-button clearance. TypeScript, production build and all 79 existing regression tests passed. No payment or backend logic changed.
